...
\end{align}
where $\nabla_2 k(x,z)$ denotes the gradient of $k$ w.r.t. $z$. The overall computational cost of such algorithm is $O((M+N)N)$ and it uses $O(M+N)$ in memory. The computational cost becomes  $O(M+N)$ when the kernel is approximated using random features as is the case for regression with neural networks (\cref{subsec:training_neural_networks}). 
This is in contrast to the cubic cost of the flow of the Kernelized Sobolev Discrepancy \cite{Mroueh:2019} which requires solving a linear system at each iteration.  
Moreover, implementing \cref{eq:euler_maruyama} is straightforward as only requires evaluating the gradient of $k$ on the current particles and target samples. This can be compared the algorithm in  \cite{csimcsekli2018sliced} which involves computing empirical CDF and quantile functions of random projections of the particles.
