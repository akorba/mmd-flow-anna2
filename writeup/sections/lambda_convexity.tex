...
\begin{proposition}
	\label{prop:lambda_convexity} Suppose $\sup_{(x,y) \in \X, (i,j) \in \llbracket 1, d \rrbracket} \partial_{x_{i}}\partial_{x_{j}}\partial_{x'_{i}}\partial_{x_{j}'}k(x,y)\le \lambda$  holds for some $\lambda \in \R^+$. Then for all $\nu, \nu'\in \mathcal{P}_2(\X)$ and any \textit{displacement geodesic} $(\nu_t)_{1\leq t\leq 1}$ from $\nu$ to $\nu'$ with velocity vectors $(V_t)_{0\leq t\leq 1}$ the following holds:
	\begin{equation}
	...
	\begin{align}\label{eq:lambda}
		\Lambda(\nu,V) = \Vert \int V(x).\nabla_x k(x,.) \diff \nu(x) \Vert^2_{\mathcal{H}} - \lambda\F(\nu)^{\frac{1}{2}}  \int \Vert  V(x) \Vert^2 \diff \nu(x) 
	\end{align}
	\end{proposition}
\cref{prop:lambda_convexity} can be obtained by computing the second time-derivative of $\F(\nu_t)$ which is then lower-bounded by $\Lambda(\nu_t,V_t)$ (see \cref{proof:prop:lambda_convexity} for a full proof).
