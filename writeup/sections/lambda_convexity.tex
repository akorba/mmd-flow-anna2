...
\begin{proposition}
	...
	\begin{align}\label{eq:lambda}
		\Lambda(\rho,v) =\left\Vert \int v(x).\nabla_x k(x,.) \diff \rho(x) \right\Vert^2_{\mathcal{H}} - \lambda\F(\rho)^{\frac{1}{2}}  \int \Vert  v(x) \Vert^2 \diff \rho(x) 
	\end{align}
	\end{proposition}
...
\begin{theorem}\label{th:rates_mmd}
	Consider the sequence $\nu_n$ obtained from \cref{eq:euler_scheme} and let us denote $\bar{K}$ the average value of $(K(\rho^j))_{0\leq j \leq n}$ over iterations. \aknote{show that $\bar{K}$ is bounded!!} If $\gamma \leq 1/L$, where $L$ is the Lipschitz constant of $\nabla k$, then:
\begin{align}
