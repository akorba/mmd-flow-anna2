...
\begin{theorem}\label{th:rates_mmd}
	Let $\bar{K}$ be the average of $(K(\rho^j))_{0\leq j \leq n}$. \aknote{show that $\bar{K}$ is bounded!!} Under \cref{assump:diff_kernel,assump:lipschitz_gradient_k,assump:bounded_fourth_oder} and if $\gamma \leq 2/3L$:%, where $L$ is the Lipschitz constant of $\nabla k$, then:
\begin{align}
