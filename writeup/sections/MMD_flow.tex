...
\int \Vert \nabla f_{\mu,\nu^{*}}(x)\Vert^2 \diff \nu^{*}(x) =0  
\end{align}
Condition \cref{eq:equilibrium_condition} does not necessarily imply that $\nu^{*}$ is a global optimum (\cite[Th. 6]{mei2018mean} and \cite{rotskoff2019global}), however. 
...
We next exploit a Lojasiewicz type inequality to prove the convergence to the global optimum provided that a particular quantity remains bounded at all times. 
