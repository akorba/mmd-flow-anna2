...
For a fixed target distributions $\mu$, we will consider a flow $(\rho_t)_{t>0}$ as described in \cref{sec:gradient_flows_functionals} and the loss functional defined as:
\begin{equation}
...
\end{equation} 
where $f_t= \int k(.,z)\diff \mu - \int k(.,z)\diff \rho_t$. 
