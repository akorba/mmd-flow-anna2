...
We are interested in analyzing the asymptotic properties of the gradient flow of the MMD. The main challenge is the lack of convexity of the MMD w.r.t. the metric structure of $\mathcal{P}_2(\X)$ endowed with the  Wasserstein distance. We show in \cref{sec:Lojasiewicz_inequality} that convergence to the global optimum happens provided that some energy functional remains bounded at all times. In  \cref{subsection:barrier_optimization}, we show that when the previous criterion fails  to hold, one can still optimize the MMD up to some barrier using a weaker notion of convexity.
