...
We are interested in analyzing the asymptotic properties of the gradient flow of the MMD. The main challenge is the lack of convexity of the MMD w.r.t. the Wasserstein metric structure. In  \cref{subsection:barrier_optimization}, we show that $\F$ is only $\Lambda$-convex and that standard optimization techniques only provide a lose bound on its asymptotic value.
In \cref{sec:Lojasiewicz_inequality}, we exploit a Lojasiewicz type inequality to prove the convergence to the global optimum provided that some energy functional remains bounded at all times. %\subsection{MMD as a free energy}
%
