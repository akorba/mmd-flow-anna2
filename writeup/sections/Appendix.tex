...
\end{align}
We would like to apply the same approach as in \cref{sec:Lojasiewicz_inequality} to provide a condition on the convergence of \cref{eq:birth_death}. We propose to first compute a linearization of the divergence $D_{\alpha}(\nu,\nu') =W_2^2(\nu,\nu') + \alpha KL(\nu\Vert \nu')$. As seen in \cref{sec:Lojasiewicz_inequality}, the Negative Sobolev distance linearizes $W_2(\nu,\nu')$. We only need to provide a linearization to $KL(\nu\Vert \nu()^{\frac{1}{2}}$. For $0<\epsilon\leq 1$ and $\nu$ absolutely continuous w.r.t to $\nu'$:
\begin{align}
	\frac{1}{\epsilon}(KL(\nu \Vert (\nu'+\epsilon ) )^{\frac{1}{2}} - KL(\nu \Vert )^{\frac{1}{2}})
\end{align}
By duality, one can introduce a divergence which  
