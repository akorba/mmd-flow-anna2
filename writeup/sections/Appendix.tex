...
\begin{assumplist}
	...
	\item \label{assump:lipschitz_gradient_k} $\Vert  \nabla k(x,x') - \nabla k(y,y')\Vert \leq L(\Vert  x-y\Vert + \Vert x'-y' \Vert ) $ for all $x,x',y,y' \in \X$.
	\item \label{assump:quad_growth}: $\Vert \nabla k(x,z) \Vert \le C \Vert x\Vert^2 $ for any $z \in \X$.
\end{assumplist}
...
\end{lemma}
\begin{proof}
	Recall that $\rho_t$ is given by $\rho_t = (s_t)_{\#}\pi$ with $s_t$ defined in \eqref{eq:convex_combination}. By $\Lambda$-convexity of $\mathcal{F}$ the following inequality holds:
	\begin{align*}
...
\end{proposition}
\begin{proof}
	The expression of the witness function is given in \eqref{eq:witness_function}. To establish \eqref{eq:gradient_witness}, we simply need to check that:
	\begin{equation}\label{eq:exchange_kernel_int}
	\nabla \int k(x,z) \diff \nu(x) = \int \nabla k (x,z) \diff \nu(x) \text{ for all }\nu \in \mathcal{P}_2(\X).
	\end{equation}
	Then, by assumption \cref{assump:quad_growth}:
	\begin{equation}
	|\nabla k(x,z)|\le \Vert x\Vert ^2 \text{ with } \int \Vert x \Vert \diff \nu(x)<\infty
	\end{equation}
	since $\nu \in \mathcal{P}_2(\X)$. Then, by the theorem of the differentiation under the integral sign, we obtain \eqref{eq:exchange_kernel_int}.
	
	To prove the Lipschitzness of $(z,\mu,\nu)\mapsto f_{\mu,\nu}(z)$, we begin by the decomposition:
	\begin{align}\label{eq:decompose_lipschitz}
	\Vert \nabla f_{\mu,\nu}(z) - \nabla f_{\mu',\nu'}(z')\Vert &\leq \Vert \nabla f_{\mu,\nu}(z) - \nabla f_{\mu,\nu}(z')\Vert + \Vert \nabla f_{\mu,\nu}(z') - \nabla f_{\mu,\nu'}(z')\Vert\\
	&\phantom{{}=\leq \Vert \nabla f_{\mu,\nu}(z)}+\Vert \nabla f_{\mu,\nu'}(z') - \nabla f_{\mu',\nu'}(z')\Vert\notag
	\end{align}
	For the first r.h.s. term we have by \cref{assump:lipschitz_gradient_k}:
	\begin{multline*}
	\Vert \nabla f_{\mu,\nu}(z) - \nabla f_{\mu,\nu}(z')\Vert =\\
	\Vert \int (\nabla_1 k(x,z)-\nabla_1(k,z')) \diff \mu(x) - \int (\nabla_1 k(x,z)-\nabla_1(k,z'))\diff \nu(x)\Vert \le 2L \Vert z-z'\Vert
	\end{multline*}
	The second r.h.s. term can be written:
	\begin{multline*}
	\Vert \nabla f_{\mu,\nu}(z') - \nabla f_{\mu,\nu'}(z')\Vert = \Vert \int \nabla_1 k(x,z')\diff \nu'(x) - \int \nabla_1 k(x,z')\diff \nu(x) \Vert\\
	\le L \Vert \int \frac{\nabla_1 k(x,z')}{L}\diff \nu'(x) - \int \frac{\nabla_1 k(x,z')}{L}\diff \nu(x) \Vert \le L W_2(\nu,\nu')
	\end{multline*}
	where we used the dual definition of the 1-Wassertein distance:
	\begin{equation*}
	W_1(\nu,\nu')=\sup_{g \text{ is 1-Lipschitz}} | \int g(x)\diff \nu(x) -\int g(x)\diff \nu'(x)| 
	\end{equation*}
	as well as the fact that $W_1(\nu,\nu')\le W_2(\nu,\nu')$ for all $\nu,\nu' \in \mathcal{P}_2(\X)$ (see \cite{peyre2019computational}). By the same computation on the last term at the r.h.s. of \eqref{eq:decompose_lipschitz} we obtain \eqref{eq:lipschitz_grad_witness}. 
...
By \cref{prop:grad_witness_function}, we directly get $\dot{\F}(\rho_t) = \int \nabla f_{\mu,\rho_t}(\psi(x,u)+t\phi(x,u)) \phi(x,u)\diff q(x,u)$.
 We shall control now the difference $\vert \dot{F}(\rho_t)-\dot{\F}(\rho_{t'})\vert$. Let $0\leq t,t'\leq 1$, using \cref{assump:lipschitness_nabla_k} and recalling that $s_t(x,u)-s_{t'}(x,u)= (t-t')\phi(x,u)$ a simple computation shows:
\begin{align*}
