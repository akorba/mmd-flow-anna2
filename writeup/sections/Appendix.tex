...
where $\hat{f}(\mu,\nu_n)$ is given by:
\[
\hat{f}_{\mu,\nu_n}(x_n^i) = \frac{1}{n_b} \sum_{b=1}^{n_b} (\frac{1}{N}\sum_{j=1}^N \psi(z_b,x_n^{j})-\frac{1}{M}\sum_{j=1}^M \psi(z_b,\xi_n^{j}))\nabla_{x_n^{i}}\psi(z_b,x_n^{i}))
\]
...
\begin{algorithm}\label{subsec:pseudocode}
	...
	\begin{algorithmic}[1]
		...
		\State \textit{Evaluate  vector field}:$\nabla f_{\hat{\mu},\hat{\nu}_n}(\widetilde{x}_n^i) = \frac{1}{N}\sum\limits_{i=1}^N \nabla_2 k(x_n^i,\widetilde{x}_n^{i})-\frac{1}{M}\sum\limits_{m=1}^M \nabla_2 k(y^m,\widetilde{x}_n^{i})$
