...
\end{equation*}
which proves the first half of the framing. Then, we also have by Cauchy-Schwartz inequality:\aknote{not clear}
\begin{align*}
\int \ps{\phi(z,\mu)-m(z),\phi(z,\mu)- \phi(z,\nu)}\diff s(z)&\le\int \Vert \phi(z,\mu)-m(z)\Vert^2 \Vert \phi(z,\nu)-\phi(z,\mu)\Vert^2 \diff s(z)\\
&=2 \mathcal{L}(\mu)^{\frac12}\mathcal{F(\nu)}
\end{align*}
