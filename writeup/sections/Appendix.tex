...
\begin{assumplist}
	\item \label{assump:lipschitness_nabla_k} $\nabla k(x,x') - \nabla k(y,.)\le \alpha \Vert x-y \Vert^2$ for any $x,y \in \X$.
	...
	\item \label{assump:lipschitz_gradient_k} $\Vert  \nabla k(x,x') - \nabla k(y,y')\Vert \leq L(\Vert \Vert x-y\Vert + \Vert x'-y' \Vert   \Vert) $ for all $x,x',y,y' \in \X$.
\end{assumplist}
...
\begin{proof}[Proof of \cref{prop:lojasiewicz}]\label{proof:prop:lojasiewicz}
	...
	\begin{align}
	\Vert \nu_t - \mu\Vert_{\dot{H}^{-1}(\nu_t)}\geq \vert \int g \diff \nu_t - \int g \diff \mu  \vert
	\end{align}
	Now, recall the definition of $g$, which implies that
	\[
	\vert \int g \diff \nu_t - \int g \diff \mu  \vert = \Vert \nabla f_{\mu, \nu_t}\Vert^{-1}_{L_2(\nu_t)} \vert \int f_{\mu, \nu_t}\diff \nu_t-\int f_{\mu, \nu_t} \diff \mu \vert.
	\]
	But since $f_{\mu, \nu_t}$  is exactly the witness functions between $\nu_t$ and $\mu$, it follows that $\int f_{\mu, \nu_t}\diff \nu_t-\int f_{\mu,\nu_t}\diff \mu = \Vert f_{\mu, \nu_t}\Vert^2_{\kH}$. We thus get the desired Lojasiewicz inequality \eqref{eq:inequality_neg_sobolev}.\aknote{make this passage clearer} Then, by 
	using $\Vert \mu - \nu_t \Vert_{\dot{H}^{-1}(\nu_t)} \le C$ and \cref{prop:decay_mmd}, we have that:  
	\begin{align}
	\dot{\F}(\nu_t) = - \Vert \nabla f_{\mu, \nu_t} \Vert^2_{L_2(\nu_t)} \leq  -\frac{4}{C}\F(\nu_t)^2	
	\end{align}
...
Under \cref{assump:lipschitz_gradient_k}, the witness function $f_{\nu,\nu'}$ between any probability distributions $\nu$ and $\nu'$ in $\mathcal{P}_2(\X)$ is differentiable and satisfies:
\[
\nabla f_{\nu,\nu'}(x) = \int \nabla k(x,x')\diff \nu(x') - \int \nabla k(x,x')\diff \nu'(x') \qquad \forall x\in \X
\]
Moreover, the map $(x,\mu,\nu)\mapsto f_{\mu,\nu}(x)$ is Lipschitz with:
\[
\Vert \nabla f_{\mu,\nu}(x) - \nabla f_{\mu',\nu'}(y)\Vert \leq 2L (\Vert x-y \Vert + W_2(\mu,\mu') + W_2(\nu,\nu')) 
\]
Finally, each component of $\nabla f_{\nu,\nu'}$ belongs to $\kH$.
\end{proposition}
\begin{proof}\manote{write proof}
