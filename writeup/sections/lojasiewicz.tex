...
\begin{proof}
	...
	%	\dot{\F}(\nu_t) \leq  -\frac{4}{C}\F(\nu_t)^2
	%\end{align}
It is clear that if $\mathcal{F}(\nu_0)>0$ then $\F(\nu_t)>0$ at all times by uniqueness of the solution.\asnote{If $\F(\nu_t)=0$ at some $t$, then $\forall t' \geq t, \F(\nu_{t'})=0$ right? So the Proposition should be Either there exists $t$ such that $\F(\nu_t)=0$ (in which case finite time convergence happens) Or convergence with rate $O(1/t)$} Hence, one can divide by $\F(\nu_t)^2$ and integrate the inequality from $0$ to some time $t$. The desired inequality is obtained by simple calculations.
\end{proof}
