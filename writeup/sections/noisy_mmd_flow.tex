...
Although the MMD flow in $W_2$ decreases in time, it can very well
 remain stuck in local minima. This can happen for instance when the negative Sobolev norm in \eqref{eq:inequality_neg_sobolev} is not finite at all times $t$. One way to see this, is by looking at the equilibrium condition for \cref{eq:time_evolution_mmd}.
...
We show in the following proposition, whose proof is provided in \cref{proof:prop:decreasing_loss_iterations}, that \cref{eq:discretized_noisy_flow} attains the global minimum of $\F$ provided the level of the noise is well controlled as well as the step-size. %\cref{eq:discretized_noisy_flow} decreases the loss functional at every iteration provided that the level of the noise is well controlled.
...
\begin{remark}
	...
	Notice that for $\beta_n = 0$  in \cref{eq:decreasing_loss_iterations} , we recover the bound of the regular algorithm \cref{prop:decreasing_functional}. However, the interesting cases are when $\beta_n>0$.
...
A proof of \cref{prop:decreasing_loss_iterations} is provided in \cref{proof:thm:convergence_noisy_gradient} and relies on \cref{eq:control_level_noise} to get a Lojasiewicz inequality which then controls the decay of $\F(\nu_n)$. A particular case when $\sum_{i=0}^n \beta_i^2 \rightarrow \infty$ holds is when $\beta_n$ is guaranteed to be always greater than a minimal value $\beta^*>0$ while still having \cref{eq:control_level_noise}. In this case, one recovers linear convergence rates for $\F(\nu_n)$.
