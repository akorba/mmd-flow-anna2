\subsection{Convergence of the time-discretized flow}\label{appendix:subsec:convegence_time_discrete}
...
\begin{proposition}\label{prop:propagation_chaos}
Given $(X_{0}^{i})_{1\leq i\leq N}$ and $(Y^{m})_{_{1\leq m\leq M}}$
samples from $\mu$ and $\nu_{0}$ and a step-size $\gamma$ the following
holds:
...
\]
where $U_{k}^{i}$ are i.i.d standard gaussians, then the following
holds:

\[
\mathbb{E}[W_{2}(\hat{\nu}_{k},\nu_{k})]\leq \frac{1}{2}\left(\frac{1}{\sqrt{N}}(B+var(\nu_{0})^{\frac{1}{2}})e^{LT}+\frac{1}{\sqrt{M}}var(\mu))\right)(e^{LT}-1)
\]
\end{proposition}
%
\begin{proof}
Let $\bar{X}_{k}^{i}$ be the particles obtained using:
\[
\bar{X}_{k+1}^{i}=\bar{X_{k}}^{i}-\gamma\nabla f_{\mu,\nu_{k}}(\bar{X_{k}}^{i}+\beta_{k}U_{k}^{i})\hfill\bar{X}_{0}^{i}=X_{0}^{i}
\]
$\bar{X}_{k}^{i}$ follows by definition $\nu_{k}$ and $(X_{k+1}^{i},\bar{X}_{k+1}^{i})$
...
\begin{align}
c_{k+1}\leq & \frac{1}{N}\sum_{i=1}^{\sqrt{N}}(\mathbb{E}\left[\Vert X_{k}^{i}-\bar{X}_{k}^{i}-\gamma(\nabla f_{\hat{\mu},\hat{\nu}_{k}}(X_{k}^{i}+\beta_{k}U_{k}^{i})-\nabla f_{\mu,\nu_{k}}(\bar{X}_{k}^{i}+\beta_{k}U_{k}^{i}))\Vert^{2}\right])^{\frac{1}{2}}\label{eq:main_inequality_c_k_1}\\
...
&+\frac{\gamma}{\sqrt{N}}(\sum_{i=1}^{N}\mathbb{E}[\Vert\nabla f_{\mu,\bar{\nu}_{k}}(\bar{X}_{k}^{i}+\beta_{k}U_{k}^{i})-\nabla f_{\mu,\nu_{k}}(\bar{X}_{k}^{i}+\beta_{k}U_{k}^{i})\Vert^{2}])^{\frac{1}{2}})^{2}\nonumber \\
 ...
 & \leq\left(c_{k}+\gamma L(c_{k}+\mathbb{E}[W_{2}(\hat{\nu}_{k},\bar{\nu}_{k})^{2}]^{\frac{1}{2}})+\frac{\gamma}{\sqrt{N}}(\sum_{i=1}^{N}\mathcal{E}_{i})^{\frac{1}{2}}+\frac{\gamma}{\sqrt{N}}(\sum_{i=1}^{N}\mathcal{G}_{i})^{\frac{1}{2}})\right)^{2}\nonumber \\
\end{align}
...
%
\begin{lemma}
\label{lem:Control_variance}Consider the an initial distribution
