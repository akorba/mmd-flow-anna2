...
\end{align}  
As pointed out in \cite{chizat2018global,Rotskoff:2019}, the dynamics in \cref{eq:particle_dynamics} can be analyzed in the "mean-field" limit when $m\rightarrow \infty$, this leads to the so called continuity equation:
\begin{align}
	\partial_t \nu_t = \nabla.(\nu_t \nabla f_{\nu_t})
\end{align}
...
Here $F$ is the functional given by the MMD: $\F(\nu) = \frac{1}{2} MMD^2(\mu,\nu)$ while
$\frac{\delta \F(\nu)}{\delta \nu }$ is the first variation of $\F$ with respect to $\nu$. In this case it is exactly given by the witness function between $\mu$ and $\nu$:
\[
\frac{\delta \F((\theta)}{\delta \nu } =  \int k(\theta,\theta')(\diff \nu(\theta')-\diff\mu(\theta') ) 
\]
