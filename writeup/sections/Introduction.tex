...
In this paper, we investigate the gradient flow for the Maximum Mean Discrepancy. In particular, we underline the limits of the MMD flow for convergence and propose a regularized flow. The latter suggests a practical algorithm that can be used for optimizing neural networks. It generally consist on injecting noise to the particles before performing the gradient updates. We show that the resulting noisy algorithm converges to the global optimum of the Maximum Mean Discrepancy unlike the original Wasserstein-2 flow, and provide rates of convergence. %We also provide an explicit rate of convergence in the mean-field limit. 
\asnote{We will probably reviewed by Mroueh so we need to compare their results to ours}

This paper is organized as follows.  \cref{sec:gradient_flow} is devoted to deriving the MMD flow and reviewing some of its properties. 
\cref{sec:convergence_mmd_flow} investigates the convergence towards a global optimum of the MMD flow and finally \cref{sec:discretized_flow} provides guarantees for the discretized flow (in time and space) of converging towards the continuous flow. 