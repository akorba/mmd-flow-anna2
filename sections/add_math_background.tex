...
\subsubsection{MMD in Reproducing Kernel Hilbert Spaces (RKHS)}\label{sec:rkhs}\aknote{to complete!!}
...
Suppose that $k(.,.))$ is measurable and that $\E_x[k(x,x)]<\infty$.  Suppose additionally that $k(.,.)$ is continuous, $\X$ is compact, and that $\kH$ is dense in $C(\X)$ the space of continuous bounded functions on $\X$. Under these conditions, MMD is a metric (see \cite{Gretton: 2012}, Theorem 5).
