...
\subsubsection{Proof of Proposition~\ref{prop:mmd_flow}}
...
of $\delta_{t}=(I+t\phi)_{\#}\nu$ we can write:\aknote{$\pi_t$?ca doit correspondre au petit paragraphe en dessous avec le début d'explication}
\[
...
\begin{proof}
	...
	The function $t\mapsto \mathcal{F}(\rho_t)$ is twice differentiable, hence one can use a Taylor expansion with integral remainder to get:\aknote{$\rho_1,\rho_0$?}
...
\begin{proof}
	...
	This follows from a simple computation which shows that:\aknote{y'a pas une erreur là? ou c'est moi, j'ai pas la même chose}
	\begin{align*}
